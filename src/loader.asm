...
; -----------------------------------------------------------------------
; load_enc_fix_payload — BUILD_TESTS_ENCODE only (the enc-tests boot
;                        variant, i234).  HLOAD the encode_inst fixture
;                        data payload into physical page 11 via the
;                        trampoline.
;
...
;
; Structural twin of load_offaxis_cluster above: HGTHD +
...
; -----------------------------------------------------------------------
; UIFA name block for "enc_fix" (BUILD_TESTS_ENCODE only).
;
; The on-disk catalogue entry is created by build-disk when invoked
