...
; name, its in-repo path, its pinned SHA-256 (the trusted hash the fetch verifies
; the proxy-served bytes against — cdn.githubraw.com is untrusted), and its size
; in bytes (a 32-bit count).  The SHA-256s are the content of each file AT THIS
; COMMIT; a fetch whose bytes hash to anything else is rejected.  The size is
; metadata the downstream consumers need: the picker UI (show each file's size),
...
; 16-bit defw (low, high) — a self-checking form (the host test re-derives the
; 32-bit value and compares it to the Go authority).
FW_MANIFEST:
                defw fw_nm_licence
                defw fw_pp_licence
...
;      4 the 32-byte SHA-256, 36 the 32-bit LE size).  Clobbers A, DE, HL.
; ===========================================================================
fw_manifest_entry:
                ld      hl, FW_MANIFEST
                ld      de, FW_REC_LEN
