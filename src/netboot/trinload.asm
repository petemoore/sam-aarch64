; trinload.asm — Quazar Trinity Network Code Loader (vendored, verbatim).
;
; PROVENANCE: this file is Simon Owen's trinload from simonowen/trinload
...
; ReadMe.txt). It is the Trinity network code loader: it listens on UDP port
; 0xEDB0 for a three-message push→run→return protocol (?/@ /X), loads a pushed
; binary into SAM RAM, executes it, and returns to its own read_loop when the
; pushed program RETs. We use it as the final-push vehicle for delivering and
; running payloads over the network (i129/i132).
;
; This file includes the already-vendored encdrv.asm + eeprom.asm in this
; directory; there is no duplicate driver copy. trinload re-uses Simon's own
; encdrv.asm byte-for-byte (verified: our copies at src/netboot/encdrv.asm +
; eeprom.asm differ from simonowen/trinload only in the provenance comment
; header we prepended; comments do not assemble).
;
; HOST VERIFIABLE: the push→run→return cycle (?/@ /X protocol) runs under the
; i80 koron-z80 harness. tools/netboot-oracle/z80/trinload_test.go assembles
; this file, injects the three protocol messages, asserts the `!` discovery
; reply egresses, the `@` data lands at the correct RAM offset (offset + LDIR
; via ROM_LDIR stub), HMPR was set to the expected page, and a pushed test
; program RETs back to trinload's start. Emulation-verified is NOT hardware-
; verified — the final integration gate stays real Trinity hardware (CLAUDE.md §5).
;
; --- begin verbatim simonowen/trinload trinload.asm @ a4b7af7 ---
