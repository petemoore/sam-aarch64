...
Both variants link at `org &8000` and must stay below the `&C000` stack-page cliff — `scripts/check-code-budget.sh` enforces this at the tail of each build and via `make check-budget`. See `docs/notes/memory-layout.md`.
...
|------|----------|-------------|-------|
| `test_mem_offaxis.asm` (wraps `test_mem.asm`) | `build/test_mem.bin` (~780 B) | physical page 13 | Largest suite (memory-operand encoders). Plan-PR 3, `docs/plans/2026-05-28-plan-pr3-test-corpus-off-axis.md`. |
| `test_offaxis_cluster.asm` | `build/test_cluster.bin` (~1225 B) | physical page 12 | "M5 + misc encoder" cluster: wraps `test_slots`, `test_pc_rel`, `test_directives_m5`, `test_ror_imm`, `test_shifted_reg`, `test_extended_reg`, `test_litpool` behind a dispatcher. `docs/notes/2026-05-29-test-variant-budget-relief.md`. |
