...
; Clobbers: HL (PC fetched).  A, BC, DE preserved.
; ---------------------------------------------------------------------
emit_byte:
                push    de
                push    bc
...
emit_bytes_n_loop:
                ...
                ld      (OUT_PC), de
; Recompute OUT_LEN += original A.  HL holds something else now; reload
; the count from the caller pattern... actually safer: just recompute
; from the diff.
                ld      hl, (OUT_BASE)
                ex      de, hl              ; HL = current PC, DE = OUT_BASE
                or      a
                sbc     hl, de
                ld      (OUT_LEN), hl
                ret
