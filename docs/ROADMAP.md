...
- **Last landed (Phase-3 design strand):** i82 ✅ (#210 — `tftp-protocol-research.md`: TFTP RFC corpus + trinload reuse-map/client-delta); i84 ✅ (#211 — `phase3-delivery-design.md`); i81 ✅ triaged. **Then 2026-06-14 (#213): Pete reviewed + resolved q11/q12, design CONFIRMED.** Revised `phase3-delivery-design.md`: **Model A only** (bare-Pi PXE pull); SAM = **DHCP+TFTP server** serving firmware + `spectrum4.img` **by name from one flat Trinity store** (**model-agnostic** — all aarch64 TFTP-boot Pis, Pi 3 + Pi 400 first, no per-machine SAM config); **both topologies** (direct cable + shared LAN, several Pis); **on-SAM HTTP self-provisioning** via uIP (i70, route decided); bootstrap via Pete's **patched SAM ROM** (i87). New items **i86** (minimal DHCP responder — mandatory: Pi-3 netboot has no static path) + **i87** (dump/reverse the patched ROM). Colin dependency dropped (materials in hand). Implementation oracle = a real Pi DHCP+TFTP packet capture (Pete has Mac netboot logs to provide).
- **In flight:** nothing.
- **AWAITING PETE (his calls):** (a) **final go-ahead to start Phase-3 implementation** (design now confirmed; q11/q12 resolved) + provide the **Mac DHCP/TFTP capture logs** (the implementation oracle) and, when his SAM is next online, the **patched-ROM dump** (i87); (b) the **i81 contributions** are community-facing so Pete submits — the aarch64 PNG-endian one-liner upstream, then a short #28 comment.
- **NEXT ACTION (autonomous, no Pete input needed):** **M9 editor** — drive the editor rendering design via the config lab (`tools/editor-prototype`), and the **i78** blank-line/comment-structure design (blank-run encoding designable now; indentation half gated downstream of i76 sign-off). Phase-3 *implementation* additionally needs i80 (SimCoupé Trinity-net emulation) for a host inner loop, or real-Trinity hardware. i77 parked (editor era).
