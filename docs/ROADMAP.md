...
- **★ Last landed (autonomous run, 2026-06-15 — 10 PRs, #279–#288):** **(a)** the **host-verifiable i99/i100 firmware self-provisioning + record-spanning storage backend is COMPLETE** — the header-skip `bodySink` Z80 port (#279), the per-file download-plan path builder + `FW_HOST` (`fw_plan_path`, #280), the per-file `FW_MANIFEST` sizes (#281), the q16 record-spanning **Go authority** `bdos.SpanPlan` (#282) + its **Z80 port** `fw_span.asm` (#283) — all host-verified vs the Go authorities, all standalone leaves (bootable image byte-identical, `&FA52`). **(b)** **i88 TLS crypto** (Pete's LOWEST-priority "build only when nothing else is left" work, correctly-ordered now the host-verifiable netboot work drained): the complete **key-schedule layer** **HMAC-SHA256** (#284) → **HKDF** (#285) → **HKDF-Expand-Label** (#286), plus **all of ChaCha20** — block (#287) + stream (#288) — each host-verified vs Go/RFC KATs (Expand-Label anchored to the independent RFC 8448, ChaCha20 to RFC 8439), all standalone leaves. The landed i88 primitives live in `src/netboot/{sha256,hmac_sha256,hkdf,hkdf_expand_label,chacha20}.asm`.
- **Earlier this run + milestone (history in `git log` + the iN/qN registries):** the i70 HTTP **wire path** (#254–#259) + **stream+verify** + the SHA-256 primitive (#269/#271) + the i100 downloader Go-authority bricks + the DHCP/TFTP netboot server + the i80 SimCoupé Trinity-net emulation. The netboot **oracle** (`tools/netboot-oracle/`) is the byte-for-byte porting spec.
- **In flight:** nothing.
- **NEXT ACTION (autonomous, FRESH CONTEXT):** continue **i88** — the remaining crypto is **Poly1305** (the MAC: 130-bit modular arithmetic, reduction mod 2¹³⁰−5 — needs an 8×8 multiply + a multi-precision schoolbook multiply) then **X25519** (the ECDH: 255-bit field, the Montgomery ladder). These are the **two biggest from-scratch multi-precision ports in the project** — verify each vs the RFC 8439 §2.5.2 / RFC 7748 KATs; keep them standalone leaves (bootable image byte-identical). After both: the TLS-1.3 handshake state machine. **This run wound down at the ChaCha20-complete boundary specifically to give these two ports fresh context** (the loop's designed context-limit mechanism — they are too large/bug-prone to start safely deep in a session).
...
- **AWAITING PETE (non-blocking — agents work other items):** q13 (editor default rendering config — taste call); i87 patched-ROM dump + i89 Pi-3 capture (hardware artifacts); i81 (community contributions, Pete submits); q14 (live-test the i97 autonomous-loop monitor). i7 phase **D=i74** stays gated (do not start); i77 parked; don't pivot to the editor (M9/i78/q13 — Pete's interface thinking pending).
