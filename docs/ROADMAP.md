...
- **Last landed (Phase-3 design strand, 2026-06-13→14):** i82 ✅ (#210 research note), i84 ✅ (#211 design), i81 ✅ triaged; **#213** — Pete resolved **q11/q12**, design **CONFIRMED** (Model A; SAM = DHCP+TFTP server serving by-name/model-agnostic; both topologies; on-SAM HTTP provisioning; patched-ROM bootstrap; new i86/i87; Colin dropped). **This PR:** Pete's real Pi 400 captures (`~/tftp-logs`) → `docs/notes/pi-netboot-capture-analysis.md`, the **DHCP+TFTP server oracle** (exact option-43 `Raspberry Pi Boot` blob, OACK `tsize`/`blksize`, serve-by-name + ERROR(1)-tolerant probe; the served-file *set* varies — the mechanism is the invariant). New items **i88** (HTTPS-from-GitHub firmware fetch — stretch, the hardest brick, lowers the contributor barrier) + **i89** (Pi 3 capture, future).
- **In flight:** nothing.
- **AWAITING PETE (his calls — none block the next agent):** (a) **go-ahead to start Phase-3 implementation** (design confirmed + the capture oracle is in hand); (b) future hardware artifacts when convenient — the **patched-ROM dump** (i87) and a **Pi 3 capture** (i89), both non-blocking; (c) **i81 contributions** are community-facing so Pete submits — the aarch64 PNG-endian one-liner upstream, then a short #28 comment.
- **NEXT ACTION (autonomous):** write the **i82/i83/i86 implementation plan** (the confirmed design + the capture oracle make this the natural next step — *plan only*; the network code cannot be verified without **i80** SimCoupé Trinity-net emulation or real Trinity hardware). In parallel, the standing **M9 editor** rendering design (`tools/editor-prototype`) + the **i78** blank-run design. i77 parked (editor era).
