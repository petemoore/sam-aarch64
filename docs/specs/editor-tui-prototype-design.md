...
**Status:** spec drafted, awaiting Pete's approval (spec gate). · **Item:** i76 ·
...
- `Set(x, y int, ch byte, ink, paper uint8)` — one character cell. `ch` is
  the SAM-displayable repertoire (ASCII 32–127 plus the block/UDG range we
  choose to use); no Unicode.
...
- No terminal affordances leak through: no bold/italic/underline, no 24-bit
  colour, no cursor styling beyond what a SAM editor would draw itself (the
  cursor is a cell drawn in reverse ink/paper, by us).
...
- **PNG at exact mode geometry.** The cell grid is drawn with a real bitmap
  font (8×8: the SAM ROM character set; 6×6/6×8: a font we author) onto the
  mode's true pixel grid — 256×192 (MODE 4) or 512×192 (MODE 3) (TM p15) —
...
§8 R2) | `mode3-6x8` | `mode3-6x6` | `mode4-8x8` | `mode4-6x8` | `mode4-6x6`.
Every layout decision in the prototype reads `Size()`; nothing hardcodes a
width. Switching geometry IS the i6 experiment: live with each for a session,
compare, decide.
...
1. **Long records vs `len: u8`.** i41 sketches a per-line `len: u8/u16`
   ...
   u8 is insufficient; the spec should pin u16 (see §8 Q2).
...
Plus the §2.4 font sheets and at least one SCREEN$ on an `.mgt`.
...
ls build/mockups/   # one PNG + one SCREEN$ per geometry, same frame, aspect-correct
```

Exit criteria: viewer scrolls the whole release smoothly at every §3
geometry; sheets delivered; i5 satisfied; i6 has its evidence base.
...
- **Z80 code.** Nothing here runs on the SAM (the SCREEN$ files are data).
...
## 8. Recommendations + the comment-width data + open questions
...
**Recommendations (not questions — proceeding unless overridden):**
...
**Genuinely open for Pete:**

- **Q1 — confirm R2's default** (MODE 3 64×24 first), or direct otherwise —
  this orders the P1 work but forecloses nothing (the matrix is one flag).
- **Q2 — i41 record length field**: pin `len` to u16 (or define an editor
  max-line policy) given the measured 1,693-char outlier; u8 (i41 §2.3
  sketch) cannot hold the real corpus. Recommend u16, no entry cap,
  soft-wrapped display.
