...
On top of the dependency DAG, the open items form a **curated priority queue**, so
...
sequence" in `docs/ROADMAP.md` (single source of truth).

The implementation lives in `registry/priority.yaml` (ordered id list),
`tools/registry/validate.go` (`validatePriority`), and `tools/registry/gen.go`
(`genBacklog`). It ships as part of i115d Phase 4.

- **Priority order** — `registry/priority.yaml`, a committed ordered list of open
  **pullable** item ids (highest priority first). "Pullable" = a non-umbrella item
  with status `OPEN` or `IN_PROGRESS`; **umbrellas are not queue entries** (they
  are spanned by their leaves). The validator enforces the list is a **permutation
  of exactly the open pullable items** — each once, nothing missing/extra/closed,
...
- **Backlog view** — `docs/notes/backlog.md` is a generated table in priority order
  with a 1-based rank column. It is the fourth generated view alongside item-open,
  item-closed, and question-open. Regenerated by `make registry`; a drift causes
  `make registry-sync-check` to fail.
...
generated `.md` files (item open/closed + question open + backlog):
...
| subcommand | action |
|---|---|
| `validate` | run all invariants (incl. priority permutation + topo); print every error; exit 1 on any |
| `gen` | regenerate the four `.md` views + re-canonicalize the YAML in place |
| `ready` | print pullable items with all deps satisfied, in priority order |
| `dependents --id iN` | print ids that `depends_on` iN |
| `dag` | print all dependency edges (`X -> Y`, one per line) |
