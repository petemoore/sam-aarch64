...
| 12 | SAM section-window `equ`s | `src/trampoline.asm:224` (`LMPR_ENCTAB`), `:252` (`LMPR_OUT_HIGH`), `:268` (`LMPR_IN_BASE`), `:290`/`:325`/`:348` (`LMPR_*`), `:489` (`HMPR_SAVE`), `:552` | The `&0000`(section A) / `&4000`(B) / `&8000`(C) / `&C000`(D) window-to-LMPR/HMPR mapping. SAM hardware geometry, not a `.ld` concept. | B |
