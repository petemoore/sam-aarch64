...
| **i39b** | M8 Phase 2 — name-table front-coding + comment/`.global`/base-hint editor sidecars (evictable region). **Split into i39b-1 (front-coding) + i39b-2 (sidecars/page-split)** per the plan. | ⏳ in progress — i39b-1 done; i39b-2 next | design §3.5/§3.6/§3.7, §5; `docs/plans/2026-06-09-i39b-nametable-frontcoding-sidecars.md` |
| **i39b-1** | Name-table front-coding — each entry becomes `[shared_prefix_len uvarint][suffix_len uvarint][suffix bytes]` shared with the PREVIOUS name (encounter order, **no sort** so `name_id`s wired through every overlay patch + the header tables stay stable). Go `WriteFile`/`ReadFile` front-code/reconstruct; the Z80 `reader_init_skip_names` walks the front-coded entries via the existing LEB128 decoder + IN cursor (never decodes a name string). | ✅ **DONE** — PR #151 (merge `add0356`); all 15 CI checks green incl. the SimCoupé matrix + m6-release; §3 review = MERGE. Compact `.tbn` 45,189 → 44,207 B (−982 B, −2.2%); assembled binary byte-identical (GNU == Go(source) == Go(.tbn) == Z80/SAM, 21752 B); `TestCompactTbnAssembly`/`TestReleasePagedInLoad` + boot self-tests green; budget test `&BF94` (108 B headroom, skip +44 B). | PR #151; `docs/plans/2026-06-09-i39b-nametable-frontcoding-sidecars.md` (i39b-1) |
| **i39b-2** | Editor sidecars + page-group split — move name strings + comment text + `.global` provenance into an evictable sidecar region; the assembler stops mapping the name/comment page (`reader_init_skip_names` deleted → the resident-RAM win, also reclaiming i39b-1's +44 B skip). Higher-risk layout change; scope after i39b-1 lands. | 🧭 next (the resident win) | design §3.5/§3.7, §4 Format B; plan i39b-2 |
