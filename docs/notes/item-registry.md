...
| **i39b-1** | Name-table front-coding — each entry becomes `[shared_prefix_len uvarint][suffix_len uvarint][suffix bytes]` shared with the PREVIOUS name (encounter order, **no sort** so `name_id`s wired through every overlay patch + the header tables stay stable). Go `WriteFile`/`ReadFile` front-code/reconstruct; the Z80 `reader_init_skip_names` walks the front-coded entries via the existing LEB128 decoder + IN cursor (never decodes a name string). | ✅ implemented + verified locally; ⏳ branch `i39b-1-nametable-frontcoding` (PR + CI pending). Compact `.tbn` 45,189 → 44,207 B (−982 B, −2.2%); assembled binary byte-identical (GNU == Go(source) == Go(.tbn) == Z80/SAM via the harness, 21752 B); `TestCompactTbnAssembly`/`TestReleasePagedInLoad` + boot self-tests green; budget test `&BF94` (108 B headroom, skip +44 B). | `docs/plans/2026-06-09-i39b-nametable-frontcoding-sidecars.md` (i39b-1) |
