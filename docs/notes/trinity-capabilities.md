...
The microcontroller is a **Microchip PIC16F74** (`-I/P`: industrial-temp, 28-pin PDIP). [Source: Pete read the part number directly off the physical chip on his board, 2026-06-29.] The PIC16F74 is a mid-range 8-bit PIC with **4K×14 words of program flash, 192 bytes of RAM, no data-EEPROM**, and an **MSSP module (hardware SPI)** plus a USART — i.e. its host link to the SAM (port `&DC`) is the SPI slave/SSP path, and the BUSY flag (`&DC` bit 3) is a *firmware* construct of this PIC, not silicon. This small SPI-bridge firmware (≤8 KB) is the shared "one PIC" behind the SD/ENC/EEPROM BUSY contention investigated in `trinity-sd-z80-interface.md` §8u/§8v: an ENC SPI op that stalls (Simon Owen's documented ENC transmit-hang) would leave this PIC stuck mid-transaction with BUSY never cleared, which is exactly the permanent-wedge the SD-write hang needs. **Firmware extraction is almost certainly blocked**: the program flash is read over ICSP (MCLR/VPP=pin 1, PGC/RB6=pin 27, PGD/RB7=pin 28, VDD, VSS) with a PICkit, but only if the code-protect (CP) config bit is clear — on a sold commercial product expect CP set, in which case an external ICSP read returns scrambled/zero. A serial connection cannot dump it (the firmware bridges via the SSP, not the USART; no serial monitor is expected). The pragmatic substitutes for the locked firmware are **asking Colin Piggot** (the definitive source for the exact BUSY semantics) and **black-box measurement** of `&DC` bit 3 (the one port not routed through the PIC, so always readable) across the SD-write sequence.
