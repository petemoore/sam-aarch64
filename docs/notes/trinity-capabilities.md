...
**Back of the board** (`~/sam-archive/trinity-docs/photos/trinity-board-v1.1-back-IMG_20260629_141924.jpg`, Pete 2026-06-29; note: a back view is horizontally mirrored vs the front — the LAN connector is on the left here, the right on the front): a **double-sided through-hole** board — all DIP footprints (40/28/20/8-pin) + the edge connector appear as solder rows, with routing on both sides. **No dedicated ICSP/programming header is visible, and no additional back-side (SMD) components** — so the 6-IC inventory is complete (no hidden parts), and the **socketed PIC16F74 is programmed out-of-circuit** (pull-and-program in a PICkit/ZIF) rather than via an on-board ICSP header. (Fine routing isn't fully traceable at this angle; a small unmarked header can't be 100% ruled out, but none is evident.)

**Still unconfirmed (minor, not on the hang path):** whether there is a **second crystal (~25 MHz) for the ENC28J60** (only the 20 MHz near the PIC is clearly seen), and the **voltage regulator** + the **Ethernet-interrupt jumper** the manual lists. None of these would reveal the SD-write-hang datum (the PIC's BUSY/SSP firmware behaviour, not on the board surface) — they matter only for firmware-read feasibility and completeness, both now essentially settled (firmware is pull-the-PIC + almost-certainly code-protected).
