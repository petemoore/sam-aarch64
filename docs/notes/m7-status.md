...
| On-SAM disassembler (strand B) — Z80 NOP stub / PR-3 | ⚠️ PR #99 merged but wiring INCOMPLETE | `src/disasm.asm`; `docs/notes/2026-06-07-disassembler-page-placement.md` | **Code landed; wiring did not.** `src/disasm.asm` (114 B NOP stub) is built into `build/disasm.bin`, but the boot-loading and self-test wiring were added in intermediate PR-99 commits (`aeaffb3`, `a843052`) then **reverted** in the final pre-merge commit `018a8ac`. What is **missing from `main`**: (1) `DISASM_PAGE`/`DISASM_ENTRY`/`DISASM_COMM_*` constants not defined in `trampoline.asm`; (2) `load_page15_payload` not in `loader.asm`; (3) "d15" not in `build-m3-disk`; (4) `call load_page15_payload` not in `assembler.asm`; (5) `test_disasm_paged.asm` not included and `run_disasm_paged_self_tests` not called. Calling convention, comm buffer, and page-placement design are correct in `disasm.asm` comments and the design note — only the integration glue is missing. The ROADMAP's previous claim "paging mechanics proven end-to-end" was based on the intermediate state. **Blocked by the test-wiring audit (next row). Fix that first, then PR-4.** |
| Test-wiring audit + repair (blocks strand-B PR-4) | ⏳ (audit in progress 2026-06-07 #5) | `src/assembler.asm`; `src/test_*.asm`; `.github/workflows/ci.yml` | **Triggered by discovering the PR-99 disasm wiring revert.** Pete flagged concern that other tests may also be unwired. Audit approach: (1) list every `call run_*_self_tests` in `assembler.asm`'s BUILD_TESTS block — inline and via off-axis clusters; (2) list every `call run_*` in `src/test_offaxis_cluster.asm` and `src/test_mem_offaxis.asm`; (3) cross-reference every `run_*_self_tests` entry point in `src/test_*.asm` to confirm it is reached; (4) flag any symbols used in test files that are undefined in the main build (symptoms of wiring gaps). Once audit completes: (a) fix any unwired tests found, (b) re-add the disasm wiring (constants + loader + disk image + boot call + self-test), (c) verify all CI SimCoupé jobs go green. Only then is the wiring trustworthy and PR-4 can proceed. |
