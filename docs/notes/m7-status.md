...
| On-SAM disassembler (strand B) — release.s round-trip | ✅ commit f2ab814 | `tools/run-disasm-roundtrip.sh`; `tests/m6/release/release.s` | **DONE 2026-06-07 (session #2).** The 745 `.inst` entries in the original pipeline were all **data bytes** (`.word`/`.quad` tables, literal-pool entries), not missing decoders — `aarch64dec` already matched `objdump` exactly on all 3908 code-only instructions. Added `text2bin -strip-data` to produce a code-only `.tbn` and wired `release.s` into `run-disasm-roundtrip.sh`; 0 `.inst` entries, round-trip passes. Fixture count: 48. |
