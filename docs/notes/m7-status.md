...
| On-SAM disassembler (strand B) — Go side | ✅ PR #93 (disasm) ✅ PR #97 (round-trip, MERGED) ✅ PR #98 (EXTR decoder + .inst-free check, MERGED) | `docs/plans/2026-05-28-go-aarch64-disassembler.md`; `tools/aarch64dec/`; `docs/specs/2026-06-07-disasm-round-trip-design.md` | **PR #93 (Go disassembler):** 100.00% objdump match; `disasm` required check; never-mis-decode hardening. **PR #97 (round-trip, strand-B PR-2):** `BranchTarget` + `WriteAsm` + `-asm` flag + `tools/run-disasm-roundtrip.sh`; 48/48 M3-M6 instruction fixtures round-trip identically; 4 data fixtures (non-4-byte-aligned or embedded `.word`) correctly skipped; pure Go pipeline; `disasm-roundtrip` CI job + `make ci-disasm-roundtrip`; five new encodings (`udf`, `sxth`, `sxtb`, `uxtb`, `uxth`) + `.inst` support in refenc. After PR #97 merges: promote `disasm-roundtrip` to required check (14th). **PR #98 (EXTR decoder + .inst-free check):** adds `tryDecodeExtr` to `tools/aarch64dec/aliases.go` (EXTR → `ror Rd, Rn, #imms` when Rm==Rn; `extr Rd, Rn, Rm, #imms` otherwise); extends `tools/run-disasm-roundtrip.sh` with a per-fixture `.inst`-free assertion (`.inst` entries signal decode failures that the byte-compare silently passes); `dir_hword.s` added to skip list. 47/47 instruction fixtures pass, 0 `.inst` entries. **Next: PR-3 Z80 port** of the disassembler. SIMD/atomics stay declined (folds into the full-ISA strand). See `memory/feedback_disassembler_first_decouple`, `memory/feedback_align_with_binutils`. |
| On-SAM disassembler (strand B) — release.s decoder-completion gap | 🧭 | `tools/run-disasm-roundtrip.sh`; `tests/m6/release/release.s` | **Gap identified 2026-06-07.** The current round-trip test covers only the M3-M6 fixture corpus; the full spectrum4 `release.s` currently produces 745 `.inst` entries (decoder gaps, not data — confirmed by running `text2bin -flatten -strip-comments release.s | refenc | aarch64dec -asm`). **Process is self-driving:** run the release.s pipeline, take the first `.inst` entry, identify and implement that missing decoder, rerun, repeat until zero `.inst` entries. No pre-analysis needed — the test output is the work queue. When zero `.inst` entries: add `release.s` to the `run-disasm-roundtrip.sh` corpus and it becomes a required CI check. |
