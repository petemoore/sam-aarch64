...
**Items are tracked with stable `iN` ids** — see the project-wide registry at
**`docs/notes/item-registry.md`** for the authoritative id↔item map and the naming
...
## Item index — the `iN` registry

The `iN` registry has moved to its canonical, milestone-neutral home:
