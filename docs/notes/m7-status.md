...
| On-SAM disassembler (strand B) — Z80 decoder port (PR-4) | ⏳ in progress (PR #102, 2026-06-07 #6) | `src/disasm.asm`; `tools/z80-test-harness-go/disasm_oracle_test.go`; `docs/plans/2026-06-07-strand-b-pr4-z80-disassembler-port.md` | **Test-first family-by-family port** of `tools/aarch64dec/` into `src/disasm.asm`. TDD keystone: `TestDisasmOracle` drives `build/disasm.bin` standalone in a koron-go/z80 emulator and compares (mnem, operands) word-by-word vs `aarch64dec.DecodeAt` over all 5438 `release.img` words — strictly stronger than the two round-trip gates, with a **ratchet floor** keeping main green and a per-mnemonic mismatch breakdown as the worklist. `TestDisasmSelfTest` runs the boot self-test (&8003) → BC=0; the harness also enforces the `paged_call` Preserves-BC/IX/IY ABI on every word. Page-top jump table (&8000 entry, &8003 self-test) → zero padding; dispatch mirrors `DecodeAt`; unported families → `.inst`. **Ported: udf (+464), move-wide movz/movn/movk+mov (+312) → oracle 41.4% (2254/5438).** Worklist (biggest first): load/store (stp/str/ldr/ldp/ldrb/strb ≈1150), branches (bl/b/b.cc ≈380 — need the pc ABI), the rest of decodeAlias (add/sub-imm, logical-imm + the non-canonical `immr≥esize` reject, dpreg-alias, condsel), decodeSys (mrs/msr via shared `src/sysreg_names.inc`), decodeDPReg. **Open (Pete's ask):** literal `z80disasm -asm` CLI mirroring [2b/3]+[2c/3]. |
