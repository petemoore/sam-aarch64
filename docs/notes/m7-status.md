...
| Source compression / compact `.tbn` (i1) | ✅ DONE — PR1 #121 + PR2 #122 + PR3 #124 merged | `docs/specs/2026-06-08-tbn-binary-format-reference.md` (§7, authoritative); `docs/specs/2026-05-27-compact-tbn-and-disassembler-design.md` (design) | **Hybrid compact format shipped end-to-end (Go + Z80), byte-identical to GNU.** **PR1** `KindLitInsts = 0x07` (runs of fully-literal PC-invariant instructions as assembled words) + `refenc -emit-compact-tbn` (reuses `encodeInst` with a two-PC invariance guard) + decode + m6-release gate. **PR2** the Z80 `REC_KIND_LIT_INSTS` handler in `src/main_loop.asm` (pass1 advance, pass2 memcpy-to-OUT), gate's SAM side switched to the compact `.tbn`. **PR3** `KindLitData = 0x08` (runs of same-directive constant numeric data; leading `directive_id` **preserves `.byte`/`.short`/`.hword`/`.word`/`.quad`** so the disassembler round-trips spelling; symbol-bearing data stays symbolic; records split at 1016 B for the Z80 `STAGING_BUF`; LIT_INSTS+LIT_DATA share one Z80 handler). **Result: 88,644 → 51,117 B (−42.3%), 6 IN pages → 4**; budgets test &BFFD / prod &B89E. Verified in the Go harness + CI's m6-release SimCoupé gate. **PR4+ (future):** Level 3 frequency dictionary — and the bigger **i39** next-gen redesign (instruction overlay + sidecars, ~−32% more). |
...
| Parity robustness seeds (from the #87 audit) | ⏳ | `docs/notes/2026-06-08-z80-go-disasm-parity-i9.md` (i9); `docs/notes/2026-05-29-z80-go-parity-audit.md` §summary | Three follow-ups the parity audit surfaced. **(1) sysname fail-hard — ✅ DONE (i9).** Root cause: `src/sysname.asm`'s PSTATE/DC/TLBI lookup-miss paths (`:421`/`:452`/`:~488`) `jp fail` when a name is outside the SAM subset — including names Go *encodes* (those three families have no generic `Sn_` fallback in Go either, so "fail-soft" would emit non-Go bytes; the faithful fix is "extend the subset"). Fixed by bringing `src/sysreg_tables.inc` to **full Go parity** for pstate (8)/dc (9)/tlbi (32); the `jp fail` now correctly mirrors Go's error only on genuinely-unknown names. Closes the symmetric disasm `.inst` gap for free (shared table). Guarded by a strengthened `sysregs_z80sync_test.go` (now enforces *reverse* parity for those 3 families). Verified: 19 names byte-match GNU through the real prod SAM assembler; oracle still 100%; boot self-tests clean. **(2) fixed Z80 caps vs unbounded Go** — already covered by IN/OUT-ceiling + bump-arena-YAGNI; untouched. **(3) untested-form-combination sweep — ✅ DONE (i9).** `tools/z80-test-harness-go/synthetic_parity_test.go` certifies structural→empirical for cond-select (csel/csinc + cset/csetm/cinc/cinv/cneg), extended-register add/sub, and signed multiplies (smull/smaddl/smsubl/smnegl/smulh) via GNU-as ground-truth → Z80-disasm-vs-Go-DecodeAt agreement. **FINDINGS (reported, not worked around):** the sweep surfaced two real decode-only Z80↔Go disagreements — (A) `ccmp`/`ccmn`: Z80 disasm has **no handler** (Go + binutils decode it; a Z80 *decoder* port gap — encoder is fine); (B) non-alias base `csinv`/`csneg`: Z80 decodes them but Go declines (Go lacks the form; Go is itself less capable than binutils — an authority decision). Both invisible to the oracle (absent from release.img). See the i9 doc §3. `sdiv` excluded (item i35). |
...
| **i39** | Next-gen maximally-efficient compact `.tbn` encoding — instruction overlay (assembled word + zeroed bits + sparse expression overlay, unifying literal/symbolic INST into one run), header label/offset table, one `.tbn` **v2** with a contiguous evictable editor region (comments/names/base-hints/`.global` flags), name front-coding, bitfield-packing as later polish. | 📋 **design agreed — Format B** (Pete 2026-06-08; all 5 open Qs resolved, §7). Projected ~38.6 KB file / ~34.5 KB resident (−32% vs the i1 51 KB), 3 phases behind the m6-release gate. **Next: Phase 1 plan.** | `docs/specs/2026-06-08-compact-tbn-nextgen-design.md` §7 — supersedes the i1 LIT_INSTS/LIT_DATA format |
| **i40** | Assembler-side editor-region eviction (enables i39's resident-vs-file split): before assembling, write the editor-only region (comments/name-strings/base-hints/`.global` flags) — or the whole `.tbn` — to disk, reuse that RAM as OUT/scratch during the build, write the assembled binary to disk, then **reload the `.tbn` to restore the editor view**. Lets "everything in one file" coexist with minimal resident RAM. | 🧭 future (editor phase) — Pete 2026-06-08 | `docs/specs/2026-06-08-compact-tbn-nextgen-design.md` §7 (decision 1) |
...
| ~~SimCoupé upstream v1.2.16 bump~~ — **DONE (i12a)** | `docs/ROADMAP.md`; `memory/project_simcoupe_upstream_pr.md` | CI's SimCoupé now builds from upstream v1.2.16 (SHA `0e8a69f3096fe00a00a29bd2303db6a7358021ad`) which ships `-exitonhalt` natively; vendored patch removed. |
