...
| Source compression / compact `.tbn` (i1) | ⏳ PR1 (Go side) in flight | `docs/specs/2026-05-27-compact-tbn-and-disassembler-design.md` § "2026-05-29 refinement"; Pete 2026-06-08 (q2: "let's compact .tbn") | **Disassembler prerequisite landed (PR-4); Pete picked this as the next strand (q2).** **PR1 (Go side, no Z80):** `format` gains **`KindLitInsts = 0x07`** (a *run* of fully-literal instructions stored as assembled words) + reader/writer + `IsFullyLiteral`; `refenc` gains `-emit-compact-tbn` (a `.tbn`→`.tbn` transform reusing `encodeInst` with a PC-invariance guard) + the decode path (pass1 `pc += 4*count`, pass2 memcpy); the m6-release gate extended to assemble + byte-verify the compact `.tbn` and print the size delta. **Measured on release: 88,644 → 68,755 B (−22.4%)**, assembling byte-identical to GNU `release.img`; IN-buffer 92% → ~71% of the 96 KB cap (6 IN pages → 5). **PR1 ✅ merged (PR #121).** **PR2 (Z80 decode) in flight:** `REC_KIND_LIT_INSTS` (0x07) handler in `src/main_loop.asm` — pass1 `PASS_PC += 4*count` (= `BC-1`), pass2 memcpy `4*count` word-bytes to OUT via `main_emit_string_bytes`, then advance; de-dup'd a shared `advance_bc_and_walk` tail to pay the ~17 B back (test variant &BFEA → &BFFB, 5 B headroom; prod &B89C). Verified in the Go harness (`TestCompactTbnAssembly`: compact release `.tbn` → SAM OUT == release.img, no Docker); the m6-release gate's SAM side now consumes the compact `.tbn`. **PR3 (designed, not built):** `KindLitData = 0x08` — raw-bytes runs of same-directive constant numeric data, with a leading `directive_id` byte that **preserves `.byte`/`.short`/`.hword`/`.word`/`.quad`** (Pete 2026-06-08 — must round-trip the source spelling). **Measured estimate:** collapsible numeric data is 21,892 B / 31.8% of the 68,755 B compact file but assembles to 4,046 B → ~13–18 KB saved, compact `.tbn` → ~51–56 KB (−37% to −43% vs the 88,644 B symbolic; `.hword` dominates). **PR4+:** Level 3 dictionary. **New: full `.tbn` encoding reference** at `docs/specs/2026-06-08-tbn-binary-format-reference.md` (single authoritative doc, supersedes the M1 spec's format tables). Open-Q6 defaults chosen. |
...
|----|------|--------|---------|
| **i1** | Compact-`.tbn` format change (hybrid bytes/symbolic; `KindLitInsts`) | ⏳ PR1 (Go side) ✅ merged (PR #121); PR2 (Z80 decode) in flight — q2 picked it as the next strand (Pete 2026-06-08) | `docs/specs/2026-05-27-compact-tbn-and-disassembler-design.md` § "2026-05-29 refinement" |
