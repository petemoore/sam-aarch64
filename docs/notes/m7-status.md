...
| On-SAM disassembler (strand B) — Z80 NOP stub / PR-3 | ✅ PR #99 (merged 2026-06-07 #4) | `src/disasm.asm`; `docs/notes/2026-06-07-disassembler-page-placement.md` | **DONE.** 114 B standalone binary at `org &8000` on page 15 via `paged_call` (HMPR=15). Calling convention: BC=hi16, IX=lo16 (extract via `push ix; pop de` — `(ix+N)` is a memory read not a register-byte access). Comm buffer: `DISASM_COMM_MNEM &7E99` / `DISASM_COMM_OPS &7EA3`, 36 B in section B. `load_page15_payload` wired at boot (prod+test). `run_disasm_paged_self_tests` inline (calls `paged_call` — cannot go off-axis; nested `paged_call` clobbers single static HMPR save slot). Budget freed by moving `test_symbols.asm`+`test_local_labels.asm` off-axis: test **656 B**, prod **1893 B** headroom. **Next: PR-4 full decoder port.** |
