...
`release.s`). Three new i48 questions are open — none blocking i39a/PR(c) — tracked
in the **`docs/notes/question-registry.md`**: **Q-i48-1** host packaging (two CLIs +
shared lib vs one tool), **Q-i48-2** editor model for value-dependent base words with
unresolved symbols (interacts with i41), **Q-i48-3** any other GNU rewrites to forego.
