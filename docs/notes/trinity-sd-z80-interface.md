...
## 8ad. BREAKTHROUGH — the faithful rig captures a WORKING BASIC write to SD (the gold CMD24), end-to-end memory→SD→memory; the divergence vs our serve is localized (i280b-b2s DONE)

Executed §8ac's plan (capture B-DOS's own working write in the faithful Continue/WTKY2 rig). Result: **a real BASIC `RECORD 1` then `SAVE "x"CODE addr,len` now writes a Trinity SD record in emulation** — verified end-to-end. Guard: `tools/netboot-oracle/z80/bdos_save_writes_record_test.go` `TestBASICSaveWritesRecordToSD` (`SKIP_PRIVATE_TESTS`-gated).

- **The working path traced:** `SAVE` → `LINERUN` → `PTDOS` → **`HSAVE`(132)** → device-select (`&8662`) → SD write → **`CMD24`** (WRITE_SINGLE_BLOCK). The record data is written to the record's data block (e.g. block 192 for the `csdV2(0x001D59)` card), and the directory is written back (a second `CMD24@152`).
- **Verified the data is REAL, not zeros (Pete's strict round-trip):** stage a distinctive non-zero pattern via BASIC `POKE` (so it lands in the interpreter's memory map — the same map `SAVE` reads; a raw harness `mac.Write` stages into whatever page HMPR holds, which `SAVE` need not read → it silently saved zeros, which the round-trip caught). After `SAVE`, the pattern is found verbatim **inside SD block 192 at offset 9** (after B-DOS's 9-byte sector header). Then `LOAD "x"CODE <other addr>` issues **`CMD17@192`** (re-reads the same block) and the loaded bytes land in RAM (physical page 1 at `&9000`) **equal to the saved pattern** — a complete memory→SD→memory round-trip.
- **The minimal precondition is `RECORD 1` then `SAVE`** — `RECORD` selects the SD device; `SAVE` alone (no `RECORD`) issues **no SD I/O** (it falls back to the floppy). `BOOT` is **not** required for the write (Pete's BOOT hint installs B-DOS's added-BASIC-keyword hook, but in this rig `RECORD` already tokenises and executes faithfully) — **correcting §8aa's "RECORD → Nonsense in BASIC" record-select-blocker claim**; there is no Nonsense error and no keyword-hook gap to clear for the capture.
- **ROM provenance settled (Pete's question):** the emulator already loads **Colin's forked ROM** (`rom.bin`), not stock v3.0 — they differ by **140 bytes in 6 regions**, all the Trinity netboot injection (EEPROM-fetch, boot-stripes→probe, SPI reader, command-table relocation). The capture dir keeps `rom_stock_v30.bin` for the diff. The rig loads **no `.mgt`/floppy** — only `rom.bin` (forked ROM) + `eeprom.bin` (Colin's forked B-DOS 1.5t); it boots from reset (`RunBootFrom(0x0000)`) to the editor idle exactly as power-on, B-DOS resident (`DOSFLG=&1D`). So "B-DOS's own write" is fully faithful.
- **The divergence vs our serve, localized:** the WORKING path goes through `HSAVE`(132) (file save: device-select from the selected-record state, then the SD write). OUR serve calls raw **`HWSAD`(149) via `rst 8` from machine code**, which — driven through the §8o external dispatch in this same booted state — **aborts at device-select and issues NO SD command** (`hk.a` arrives 0 because the external `rst 8` path resets `A'`; §8s). So the bug is exactly Pete's framing: *B-DOS's own BASIC write works; our machine-code invocation doesn't establish the device-selection state `HSAVE` does.* **NEXT (i280b-b2t):** diff the device/sysvar state a real `RECORD 1` leaves (the gold) against what our serve's HRECORD-select establishes, and port the missing setup into `src/netboot/bdos_seam.asm` (reuse B-DOS entry points; don't reimplement) — then emulation-verify our serve's `HWSAD` reaches `CMD24`, then a TAPO hardware retest.

## 8ae. i280b-b2t — authority diff of the working vs our write path + a METHODOLOGY pivot: the §8s/§8o armed-dispatch harness is reboot-artifact-contaminated; validate the fix only in the faithful (Continue/WTKY2) rig

Started i280b-b2t (port the working setup into `bdos_seam.asm`). Did the authority diff (Colin's `bdos15a.src.txt` + `bdos15t-beta6.annotated.dis`) and an emulation probe. Two durable results — one about the fix, one about methodology.

**Authority diff (the two write paths):**
- **Working `SAVE` → `HSAVE`(132)** (`bdos15a.src.txt:405`): calls **`set.drive`** (selects the *current* device from the RECORD-set sysvars `&8132`/`&8135`), then `open.file`, then the block writes `HSVBK`/`HCFSM` → `wr.buff`. It does **not** read `hk.a` for device selection.
- **Our serve → `HWSAD`(149)** (`bdos15a.src.txt:530` / disasm `&9E16`): the prelude `&9E3C ld a,(hk.a) / &9E3F call &8662` — device-select is **keyed on `hk.a`**. `&8662`: `cp 1` → `&780B=1`; `cp 2` → Trinity SD setup (`call &8677` which gates on `&80AF`≠0, `call &A0E4`), `&780B=2`; **else → `&8680`** (`call &5A8B; ld a,(&780B); dec a; ret nz`) — which, when `&780B` is already 2 (a prior select), **returns leaving `&780B=2`** (the SD selection is *preserved*, not aborted).
- So the divergence is real (current-device vs `hk.a`-keyed), but the device-select with `hk.a=0` does **not** simply abort — the `&8680` path preserves a prior `&780B=2`. This matches the #730 reasoning ("`hk.a=0` leaves the SD select"), **contradicting** the §8s "device-select aborts on `hk.a=0`" framing.

**Emulation probe (the surprise):** drove `HWSAD` via `rst 8` with main `A=2`, no force-poke of `&81D9`/`&80AF`, through the §8s `bootWriteCoreMachine` (mount-var-set poked + RECORD). Result: `hk.a(&81D9)=0`, `&80AF=0`, yet it **issued `CMD24@153`** — i.e. **our `hk.a=0` invocation reaches a CMD24 *in emulation***. So `hk.a` is very likely **not** the discriminator (consistent with the long-standing §8m/§8o caveat that our read path HRSAD works on hardware with the same shape). The simple "set `hk.a=2`" fix is **not** supported.

**⚠️ METHODOLOGY FINDING (load-bearing for the next session):** that `CMD24@153` traced at **`PC=&01CB`** — the ROM `HLJPI` `JP (HL)` trampoline that **§8aa identified as the reboot-artifact address**. The §8s/§8o/§8q armed-dispatch harness (`bootWriteCoreMachine` → `runCmdLine` → `RunBootFrom(addrEditorIdle=&01CB)`) resumes at the **broken** `&01CB` point with a reset stack — the **same reboot loop §8aa refuted**. So the armed-dispatch harness's "write-core reachable / `hk.a` / CMD24" observations (§8s `TestHWSADWriteCoreReachableWithGatesForced`, §8o) are **suspect — contaminated by the reboot artifact**, exactly like §8z. **Only the faithful rig (Continue + WTKY2 `&04FA`, #746/#747) is trustworthy.** The §8c–§8o `hk.a`/bank conclusions reached via the armed dispatch should be re-derived in the faithful rig before being relied on.

**Why the fix is not emulation-closable as-is:** §8ac established the hang is **hardware-only** (the koron-go SD model always clears `&DC` BUSY, so our invocation issues CMD24 in emulation and never hangs). So a `bdos_seam.asm` fix **cannot be validated by "emulation now issues CMD24"** (it already does). The fix must be derived from a *faithful* comparison of our serve's device/register state vs the working `SAVE`'s, then confirmed by a **TAPO hardware retest** (Pete de-gated power-cycles; no scope needed for "does the push complete").

**NEXT (i280b-b2t, fresh context):**
1. **Re-prioritize:** `build/registry prioritize --id i280b-b2t --to-top` + `build/registry dep add --id i283 --on i280b-b2t` (i283's PIC-wedge premise is §8ac/§8ad-refuted; the hardware `&DC` measurement is the *fallback* if the b2t fix's hardware retest still hangs).
2. **Build a FAITHFUL-rig driver for a raw machine-code `HWSAD`** — inject the serve's exact `bdos_write_sector` sequence and run it via `Continue` preserving the editor-idle paging/state (NOT the armed `RunBootFrom(stub)` / `addrEditorIdle` dispatch). Compare the device sysvars (`&8132`/`&8135`/`&780B`/`&80AF`/`hk.a`) our serve leaves vs what a faithful `RECORD`+`SAVE` leaves at its CMD24. The real discriminator is in that diff.
3. If the faithful diff doesn't pin it (the hang is hardware-only), **hardware-retest the current serve** as-is (post-§8d handshake fix, post-§8ac reframe) with block-localizing markers to see *where* it now hangs — the §8d–§8w evidence predates the b2d/handshake fix and may be stale.
...
## 8ah. q62 option (a) IMPLEMENTED (emulation-first) — the serve disk push writes record sectors with our OWN self-healing CMD24 by absolute LBA, bypassing the B-DOS HWSAD/HRECORD/HRSAD hooks (i194)

q62 option (a) is built and emulation-verified. The serve's disk-record WRQ push no longer touches the flaky B-DOS hooks for the record write/select; it uses our own self-healing raw SD ops throughout. Hardware deploy stays Pete-gated (a reimplemented write to a shared physical SD card — data safety).

**What changed (all in the bootable serve build; the B-DOS hook routines stay in the tree — other paths/tests use them):**

- **`bd_record_write_hw` (sd_csd.asm)** — writes one 512-byte record-data sector by ABSOLUTE card LBA via our own CMD24, reusing the same CMD24 protocol core `bd_list_write_hw` uses. The refactor extracted that core into **`bd_cmd24_write_core`** (LBA in `bd_list_lba`, source pointer in `bd_cmd24_src`); both the list-claim write and the record-data write now share one tested CMD24 handshake. The record→block math is `LBA = csd_base + 1600*(record-1) + linearSec` (the §6/§7 record geometry; `csd_base` = first data sector from the CSD decode), with the v1 byte-shift factored into `bd_lba_apply_v1_shift` (no-op for our v2/SDHC card). Self-healing: it runs the full `sdc_init_ladder` every call, so the serve's ENC-`ereset`-disturbed SPI state is re-established before each write — the property §8ag identified as why our raw ops survive where the B-DOS hooks hang.

- **`raw_record_sink.asm`** — `rrs_flush_sector` gained a backend selector `RRS_OWN_CMD24`: `0` keeps the i122 client fetch-and-store path on the HWSAD hook (`bdos_write_record`); `1` (the serve push) writes via `bd_record_write_hw` into `RRS_RECORD` (the claimed record). `raw_record_sink_reset` clears it to 0; `handle_wrq` sets it to 1 for the disk-record class.

- **`netboot_serve.asm`** — the disk-record claim path **drops the HRECORD select** (`bdos_select_record`): the write is by absolute LBA, so B-DOS's "current record" state is unneeded, and HRECORD is exactly the hook §8ag saw hang (`CLAIM_SELECT_PRE` with no `_POST`). The flat-file class STILL HRECORD-selects (its HSAVE commit targets the current record). `wd_finalize` drops its HRSAD sector-0 read-back (validation is size-only, i285 — there is no stamp to read). The claim's record-LIST NAME entry is still written via our own self-healing `bd_list_write_hw` (`bdos_claim_record`), and the free-record find still uses our own `bd_list_read_hw` (CMD17) — both kept.

**Emulation verification (the gate):** `sd_record_write_test.go` unit-tests the record→absolute-LBA math against the §6/§7 formula for a battery of (record, linearSec) pairs incl. the record-boundary edges (0, 1599) and asserts the CMD24 lands at the exact block with the right bytes (the i145c/i145h SD model captures `store[LBA]`), plus a data-safety test that every write stays in `[csd_base+1600*(n-1), csd_base+1600*n)`. The full WRQ push tests (`netboot_serve_wrq_record_test.go`) now assert the SD model's record-data blocks (not the HWSAD hook capture) with the data-safety invariant that the push touched ONLY the claimed record's range + the legitimate record-list area; the debug-marker test (`netboot_serve_dbg_test.go`) asserts the new marker sequence (no `CLAIM_SELECT`, no `HWSAD` markers on the push path). All netboot-z80 + bdos + serve tests green; `make netboot-serve-boot netboot-serve-boot-debug netboot-bdos-seam` assemble.

**Still hardware-gated (Pete):** the koron-go SD model always clears `&DC` BUSY and does not model the shared-PIC wedge, so emulation cannot reproduce the §8ag hardware hang nor confirm the fix on silicon. The decisive test is a real cj.mgt push: with the B-DOS hooks out of the push path, the claim's HRECORD hang (§8ag) and the per-block HWSAD hang (§8af) should both be gone — but that is a write to a shared physical card via a reimplemented path, so Pete signs off the data-safety direction before the first hardware shot (`trinity_storage_shared_resource`).

Authority: this section; `src/netboot/sd_csd.asm` (`bd_record_write_hw` / `bd_cmd24_write_core` / `bd_lba_apply_v1_shift`); `src/netboot/raw_record_sink.asm`; `src/netboot/netboot_serve.asm` (`wrq_claim_record` / `wrq_arm_flat` / `wd_finalize`); tests `sd_record_write_test.go` + `netboot_serve_wrq_record_test.go` + `netboot_serve_dbg_test.go`; §8ag (the q62 framing), §6/§7 (record geometry), `bdos/raw_sink.go` (linear-sector authority).

## 8ai. HARDWARE TEST of the own-CMD24 push (option a) — the §8af/§8ag HANG IS BEATEN (no 301s stall), but the per-block full self-heal is too SLOW to finish 1600 blocks, and progress is UNOBSERVABLE (markers perturb, §8x); next = a lighter per-block re-establish + a non-perturbing progress channel (i194/i284)

Pushed the production own-CMD24 serve (#752, `bd_record_write_hw` verified in the build) and `curl -T cj.mgt tftp://…/trinity-sam-disks/cj` (819200 B, highest-free record, TAPO self-serve). Two runs:

- **The §8af/§8ag hang is GONE.** Both prior cj pushes (HWSAD path) stalled at **~301 s** with `curl 28` (the TFTP stall timeout — no progress). The own-CMD24 push ran the **full 30-minute `--max-time`** (`curl 28` at 1800 s, not a 301 s stall) — i.e. the transfer stayed alive (blocks ACKing) the whole time. So bypassing the B-DOS hooks removed the deterministic hang (strong positive result for option (a)).
- **But it did not COMPLETE 1600 blocks in 30 min.** Root cause: `bd_record_write_hw` runs the **full `sdc_init_ladder`** (wake + CMD0 + CMD8 + the ACMD41 ready-loop + CMD58/59) **on every sector**. That per-block self-heal is *why* it survives the interleaved ENC ACK (each TFTP block is ENC-RX → SD-write → ENC-TX, and the ENC op disturbs the shared SD/PIC — §8d), but the ACMD41 init handshake ×1600 is far too slow (~819 KB at the observed rate ⇒ well over 30 min).
- **Progress is currently UNOBSERVABLE.** Production has no markers; the debug serve's per-sector markers are ENC TX that itself perturbs the shared bus (§8x) and would 1600× flood; and we cannot read the card from the host. So "stayed alive for 30 min" does **not** prove sectors landed — it could be writing slowly OR ACKing while the writes no-op. **We need a non-perturbing on-hardware progress channel** (the §8ab ring-buffer / border-counter idea — NOT an ENC marker) to know.

**NEXT (i194/i284):** (1) make the per-block self-heal LIGHTER — full `sdc_init_ladder` once at record-write start, then a cheap **re-select** (re-point the PIC to SD after each ENC ACK: `OUT (&DC),&31` + settle) per sector instead of the full CMD0/ACMD41 ladder; the card stays initialised once awake, so only the SD *selection* needs re-establishing after an ENC op. **Reliability caveat:** a lighter re-establish might re-introduce the hang (the full ladder is what demonstrably survived) — this is the §8d speed/reliability tradeoff and needs hardware validation, which (2) requires a non-perturbing progress channel first (so a run's outcome is *observable*, not a 30-min black box). (3) Then a clean push should complete with `curl 0` = cj installed. The BOOT half is then SAMBOOT autoboot (i264b: write a `SAMBOOT Config` chunk → cj's record; the bootblock's boot-a-record is proven). Emulation can't help here (no shared-PIC model) — this is iterative hardware work, best with the §8x scope or the §8ab tracer. **SAM left OFF.**
...
**The drain theory (Pete) — possibly NO re-arm is needed at all:** with every routine draining (each OUT followed by its IN) and no interrupt touching the ports, sequential ENC↔SD alternation is safe by construction — the shared read latch is only a hazard for an *un-drained* peripheral switch, and the interleave detector proves we drain (zero violations). Crucially, **the i249 "serving died after the first SD read" observation predates our draining discipline** — so that death may have been the *un-drained shared-latch read itself* (the driver reading a stale byte and wedging), which proper draining now fixes, NOT a genuine ENC-RX-engine disturbance. So the test ORDER is: **(1) proper draining + ZERO re-arm** (drop `enc_rx_reestablish` entirely, just MUX-switch); if the ENC RX still genuinely needs nudging, **(2) a MINIMAL re-arm** (re-enable `ECON1.RXEN` / re-point `ERXRDPT` / re-select ENC — no `&28` reset, no reconfigure). The full `ereset` is wrong either way.

**THE PIVOT (i289):** replace the per-block full `ereset` with zero-or-minimal re-arm (i289), use B-DOS HWSAD directly (`RRS_OWN_CMD24=0`), and hardware-test the full `cj.mgt` push with **ACK-logging observability** (the Pi is the TFTP sender, so it sees every ACK block# — passive, no markers, no root). Emulation-first needs the ENC reset/re-arm modelled faithfully from the datasheet (i288) — today the model's `rxDisarmed` is cleared *only* by the full reset (it encodes the assumption under test).
