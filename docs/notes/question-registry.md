...
| **q9** | **Full-fidelity source residency strategy (i59)** — the unstripped release comment corpus is 291,038 B of body text (7,012 sidecar rows). Best Z80-feasible compression (ZX0-class LZ, 68 B decoder) reaches ratio 0.287 → ~85.5 KB, which **fits a 512 KB SAM with ~15 pages spare but misses a 256 KB SAM by one page** (needs 6, 5 free after the assembler's 11). Options: (a) target a 512 KB SAM — compressed-resident, simplest; (b) ZX0-resident + cold-block spill to floppy/SAMDOS (the i40 mechanism) for 256 KB; (c) Trinity SD spill — blocked on the port-`&DC` SD-select byte (in your paper Quazar programming manual) and ideally the SAM Revival 21 SD-driver source (your copy). Sub-asks: (1) is the target machine 256 or 512 KB? (2) can the Quazar manual's SD section be recovered/scanned? (3) is the SR21 cover-disk source recoverable? | ⏳ **OPEN — write-path decided (Pete 2026-06-10): on-SAM greedy ZX0-format compressor, see i60. Remaining sub-asks: machine RAM size / Quazar manual / SR21 disk** | i59, i60; `docs/notes/comment-compression-research.md`; `docs/notes/trinity-capabilities.md` |
