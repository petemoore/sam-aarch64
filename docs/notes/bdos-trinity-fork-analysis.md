...
The routine-by-routine SD-SPI interface — the `&DC`/`&DF` protocol, the SPI byte primitives, the CMD9/CSD-decode crux, the record→sector math, and the port-to-fresh-Z80 mapping — is the dedicated deep-dive `trinity-sd-z80-interface.md` (the port authority for items i141/i145). This doc is the overview; that doc is the interface reference.
