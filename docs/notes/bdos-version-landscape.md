...
- Development transfers between them at the hook level — **VERIFIED for the SAMDOS ↔ B-DOS AL 1.5a (Atom Lite) pair** by the i62 dual-run experiment below, and **hardware-verified for the Trinity 1.5t leg** (i93b, 2026-07-02): the `hook_roundtrip` probe (`src/netboot/hook_roundtrip.asm`) runs the full HRECORD → HSAVE → HGTHD → HLOAD → byte-compare round-trip via real RST 8 dispatch, PASSed against the captured ROM + B-DOS 1.5t in emulation (`tools/netboot-oracle/z80/hook_roundtrip_faithful_test.go`) and then against real B-DOS 1.5t on the real SAM + Trinity (scratch record, verdict P). The static analysis (i71, `bdos-trinity-fork-analysis.md`) shows why the transfer holds: the hook dispatch, the 39-entry vector table, and the handler code are all 1.5a's bytes under relocation — only the sector-device layer changed.
