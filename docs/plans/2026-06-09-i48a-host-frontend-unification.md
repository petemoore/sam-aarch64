...
> **STATUS (2026-06-09 #5):** ✅ **COMPLETE — all three PRs merged.** ✅ **PR1** (#141, `271b202`) · ✅ **PR2** (#142, `1c2c0f9`) · ✅ **PR3** (#144, `d07d627`). All byte-neutral with all 14 CI checks green (incl. the SimCoupé matrix + the m6-release 3-way byte-match) and §3 reviews = MERGE. **PR3 delivered:** the shape-b currency change (`File.Records []byte`→`[]Record`; front-end builds structs; symbolic kinds never serialized), the format-lib removal of the symbolic-kind writer methods + reader decode-arms (deferred from PR2 — a real format change, as noted below), the Decision-B strictness (items 1–3), the q7 sweep (resolved), the i48d overlay-only doc rewrite, and the M1 goldens → re-assemble-and-byte-check (36 dead goldens removed). **Post-merge cleanup (#146):** the review's `WriteRaw` follow-up plus a dead-code sweep removed `WriteRaw`, three orphaned `Reset()` helpers, and `SymbolTable.Name`/`Len`; a `staticcheck` (unused/U1000) CI gate was added to guard against re-accumulation.
