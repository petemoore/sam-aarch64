...
//
// Tests drive the real Z80 code under the koron-go/z80 harness with the
// harness CardModel attached (via the bdHookListRead hook). The tests assert:
//
...
//
// THE HONESTY LINE (CLAUDE.md §5): bdos_read_list_sector issues the harness
// hook BD_HOOK_LISTREAD = 161; the harness serves the list bytes from the
// CardModel. The hardware implementation (raw SD CMD17 SPI on ports &DC–&DF)
// is hardware-gated (docs/specs/trinity-record-detection-design.md §8,
// open-verification-item-1) and is not verified here. Emulation-verified is
