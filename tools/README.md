...
| `run-disasm-roundtrip.sh` | Drives the Go encoder/decoder round-trip gate: two pure-Go round-trips (binary + compact `.tbn` overlay render) over the M3-M6 fixture corpus and `release.s`. | `ci-disasm-roundtrip` |
