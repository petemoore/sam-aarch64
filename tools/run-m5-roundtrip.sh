#!/usr/bin/env bash
# run-m5-roundtrip.sh — end-to-end M5 fixture round-trip driver.
#
# Pipeline (mirror of run-m4-roundtrip.sh — same oracle, same disk
# format, just the M5 fixture corpus).  See
# docs/specs/2026-05-27-m5-compound-operands-directives-design.md §3.
#
#   1. text2bin INPUT.s   → INPUT.tbn
#   2. build-m3-disk assembler.bin enctab.enc INPUT.tbn → OUT.mgt
#      (the M3 assembler binary is M5-capable post-PR-A; the .tbn
#       record format is unchanged, just more directive coverage.)
...
#
# Usage: run-m5-roundtrip.sh <fixture.s>
