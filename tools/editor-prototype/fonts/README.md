...
| `font6x6.sam` | The same 96 glyphs in the loader's whole-charset layout: glyph 0 first, so chars 0–31 are 192 leading zero bytes followed by `five-pixel-font-6x6.bin` verbatim. This is the file `FontFor` reads. | derived from `five-pixel-font-6x6.bin` (same licence) |
