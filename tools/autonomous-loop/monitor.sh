...
STARTUP_PROMPT="${ALOOP_PROMPT:-Continue per docs/ROADMAP.md. AUTONOMOUS-LOOP RUN: after you complete each work item (a merged PR), run 'touch ~/.claude/autonomous-loop/task-done' and end your turn -- the monitor will then show you /context; if it reports under 50% free context, write your ROADMAP Current-State handover and then run 'touch ~/.claude/autonomous-loop/wound-down', otherwise pick the next item and continue (the task-done checkpoint nudge carries the full rule: wind down even above 50% free if the only work left is fresh-session/Pete-gated/over-budget -- grep the OPEN registries, do not judge drained from the curated Current-State menu). Never block on Pete while unblocked work remains -- put any question in the qN registry and keep working. Full protocol: tools/autonomous-loop/README.md.}"
