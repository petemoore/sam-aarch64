...
func main() {
	...
	// The mnemonic-id .inc is likewise a straight Go-package projection.
	if outMnemonicIDsInc != "" {
		out, err := os.Create(outMnemonicIDsInc)
		if err != nil {
			fail(err)
		}
		if err := emit.RenderMnemonicIDsInc(out); err != nil {
			out.Close()
			fail(err)
		}
		out.Close()
