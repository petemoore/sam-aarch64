...
`https://github.com/petemoore/sam-aarch64/blob/c0f62fa/docs/notes/2026-05-28-test-harness-bakeoff-evaluation.md`. This doc captures the
...
- **SAMDOS disk I/O**: entirely absent.  HGTHD reads nothing from disk; HLOAD does nothing; HSAVE writes nothing to disk.  The harness captures bytes directly from RAM.  Files the assembler needs must be supplied as `NamedFile`s (registry) or pre-deposited.  **A file the assembler HGTHDs but the harness was never given is, by default, a silent HLOAD no-op (leaving the target page empty) — recorded in `Result.UnservedFiles` and named in the trap/timeout message, but not halted.**  This default keeps the decode-focused prod-boot tests working (their unconditional d15/sd13/zx013 loads are harmless for fixtures needing neither).  A stranded page is a classic cause of a downstream "jumped into garbage → &0038" trap; in particular the **prod assembler unconditionally HLOADs `"sd13"` (sysreg lookup data) into page 13 at boot** — always pass `-sysreg-data build/sysreg_data.bin`.  See `https://github.com/petemoore/sam-aarch64/blob/c0f62fa/docs/notes/2026-05-29-go-harness-paged-trap-rootcause.md`.
  - **Error model — DOSER (`&5BC0`) post-hook dispatch**: after EVERY file-I/O hook (success AND error) the harness models ROM PTDOS's `JP (DOSER)` dispatch (`rom:12977-12980, 13003`) — `ld hl,(DOSER); inc h; dec h; jr nz; jp (hl)` with register `A` = the SAMDOS error number (0 on success).  `Config.DoserAddr` (defaults to `&5BC0`) points at the emulated DOSER vector.  With a handler installed: a success pushes the handler above the caller's resume slot so the handler's `and a; ret z` resumes the caller; an error overwrites the resume slot so the handler takes over with `A` = the error number.  With no handler ((DOSER)=0): a success is an ordinary hook return; an error takes the default path, surfaced as a clean cause-naming halt.  `Config.StrictFileNotFound` makes an unserved file a faithful file-not-found error rather than the silent no-op; `Config.FailHGTHD` / `Config.FailHSAVE` inject failures.  See `samdos_error_longjmp_test.go`.  The earlier `(hksp)` model is **gone** — i185 established that `(hksp)` is NOT app-usable (the SAMDOS dispatcher zeros it on every hook entry, `samdos/src/b.s:450-451`, and it is SAMDOS-internal/NMI-only); `DOSER` is the correct, app-facing error vector.  See `docs/specs/samdos-file-io.md` "Critical caveat" and registry items i25 / i185.
- **HGTHD for enctab.enc**: registered in the harness's files map so that HGTHD populates &4B50+34/35 with the true length (i7 phase A: load_enctab reads the DIFA header instead of a hardcoded constant, matching the load_payload_generic pattern).  HLOAD remains a no-op; the data is pre-deposited into page 4.
