...
- **SAMDOS disk I/O**: entirely absent.  HGTHD reads nothing from disk; HLOAD does nothing; HSAVE writes nothing to disk.  The harness captures bytes directly from RAM.  Files the assembler needs must be supplied as `NamedFile`s (registry) or pre-deposited.  **A file the assembler HGTHDs but the harness was never given is, by default, a silent HLOAD no-op (leaving the target page empty) — recorded in `Result.UnservedFiles` and named in the trap/timeout message, but not halted.**  This default keeps the decode-focused prod-boot tests working (their unconditional d15/sd13/zx013 loads are harmless for fixtures needing neither).  A stranded page is a classic cause of a downstream "jumped into garbage → &0038" trap; in particular the **prod assembler unconditionally HLOADs `"sd13"` (sysreg lookup data) into page 13 at boot** — always pass `-sysreg-data build/sysreg_data.bin`.  See `https://github.com/petemoore/sam-aarch64/blob/c0f62fa/docs/notes/2026-05-29-go-harness-paged-trap-rootcause.md`.
  - **Error model (i183)**: `Config.StrictFileNotFound` (and the `Config.FailHGTHD` / `Config.FailHSAVE` injectors) turn a failed file-I/O hook into a SAMDOS error longjmp instead of the silent no-op — longjmping into a handler vector when one is installed, or halting cleanly with a cause-naming message when none is.  See `samdos_error_longjmp_test.go`.
  - **⚠️ KNOWN FIDELITY GAP (i185 / fixed by i25): this model uses the WRONG vector.**  It reads `Config.HkspAddr` and models `derr`'s `(hksp)` longjmp — but an application **cannot** use `(hksp)`: the SAMDOS dispatcher zeros it on every hook entry before the body runs (`samdos/src/b.s:450-451`), and it is SAMDOS-internal (NMI-only).  The real app-facing vector is **`DOSER` (`&5BC0`)**, dispatched by ROM PTDOS *after* every hook (`rom:12977-12980, 13003`).  A future `(hksp)` handler would pass here but fail on hardware — an emulation-first trap.  i25 corrects this to model the `DOSER` post-hook dispatch (success + error) and treat an app-set `(hksp)` as zeroed.  The default-halt path (no handler → cause-naming halt) is correct and useful as-is.  See `docs/specs/samdos-file-io.md` "Critical caveat" and registry items i25 / i185.
