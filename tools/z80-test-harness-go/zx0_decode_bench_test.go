...
func TestZX0DecodeBench(t *testing.T) {
	...
	for _, bsz := range []int{1, 2, 4, 8} {
		...
		for _, zx0Path := range matches {
			...
			t.Logf("%-38s  raw=%5d  comp=%5d  std: %6.1f T/B  %.3f ms  turbo: %6.1f T/B  %.3f ms  mega: %6.1f T/B  %.3f ms",
				filepath.Base(zx0Path), len(rawData), len(compData),
				stdTpB, stdMs, turbTpB, turbMs, megaTpB, megaMs)
		}
	}
	...
	t.Logf("%-10s  %-10s  %-10s  %-10s  %-10s  %-10s  %-10s  %s",
		"BlkSize", "Std T/B", "Std ms/blk", "Turb T/B", "Turb ms/blk", "Mega T/B", "Mega ms/blk", "N  ratio")
	...
	for _, bsz := range []int{1, 2, 4, 8} {
		...
		t.Logf("%-10s  %-10.1f  %-10.3f  %-10.1f  %-10.3f  %-10.1f  %-10.3f  n=%d  ratio=%.3f",
