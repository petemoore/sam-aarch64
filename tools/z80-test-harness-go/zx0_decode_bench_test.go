...
func TestZX0DecodeBench(t *testing.T) {
	...
	if _, err := os.Stat(blocksDir); os.IsNotExist(err) {
		// Environmental, not a build-wiring gap: the .zx0 reference blocks are
		// produced only when the external `zx0` compressor is on PATH (see the
		// Makefile zx0-blocks target). Explicit skip, like the GNU-toolchain skips.
		t.Skipf("no ZX0 decode blocks at %s — needs the external 'zx0' compressor (make zx0-blocks)", blocksDir)
	}
	...
	if len(results) == 0 {
		// No .zx0 reference blocks present — the external `zx0` compressor is not
		// on PATH (make zx0-blocks writes .zx0 only when it is). Environmental skip.
		t.Skip("no .zx0 reference blocks — the external 'zx0' compressor is not available")
	}
