...
//
//	0x0000–0x01FF  decoder code (max 256 B for either variant)
//	0x8000         compressed block (source, HL on entry)
//	0xC000         output buffer (destination, DE on entry)
//	0xFFFE         initial stack pointer (for PUSH/POP / EX(SP),HL in decoder)
//
// # Reproduction
//
...
const (
	...
	// A trampoline at 0x0200 does CALL 0x0000 ; HALT so the decoder's final
	// RET lands at the HALT — clean, portable termination with no PC-range
	// guessing.
	zx0DecoderBase    = 0x0000 // decoder code starts here
	zx0TrampolineBase = 0x0200 // CALL 0x0000 + HALT — execution entry point
	...
	zx0InitialSP      = 0x7FFE // stack grows down; safe zone 0x0300–0x7FFD
...
// Memory layout:
//
//	0x0000–0x01FF  decoder code
//	0x0200         trampoline: CD 00 00 (CALL 0x0000) + 76 (HALT)
//	0x8000         compressed block (HL on entry)
//	0xC000         output buffer (DE on entry)
//	SP = 0x7FFE    stack grows down; ample room above 0x0300
...
func TestZX0DecodeBench(t *testing.T) {
	...
	for _, bsz := range []int{1, 2, 4, 8} {
		...
		for _, zx0Path := range matches {
			...
			t.Logf("%-38s  raw=%5d  comp=%5d  std: %6.1f T/B  %.3f ms  turbo: %6.1f T/B  %.3f ms",
				filepath.Base(zx0Path), len(rawData), len(compData),
				stdTpB, stdMs, turbTpB, turbMs)
		}
	}

	if len(results) == 0 {
		t.Skip("no results collected — run 'make zx0-blocks' first")
	}

	// Per-block-size summary.
	t.Logf("")
	t.Logf("── ZX0 decode speed summary (SAM 6 MHz, uncontended RAM) ──────────────")
	t.Logf("%-10s  %-10s  %-10s  %-10s  %-10s  %s",
		"BlkSize", "Std T/B", "Std ms/blk", "Turb T/B", "Turb ms/blk", "N  ratio")
	...
	for _, bsz := range []int{1, 2, 4, 8} {
		...
		t.Logf("%-10s  %-10.1f  %-10.3f  %-10.1f  %-10.3f  n=%d  ratio=%.3f",
