; dzx0_fast.asm — pyz80 port of the speed-optimized ZX0 decompressor
; by spke (187 bytes).
;
; Ported from reference/zx0/z80/dzx0_fast.asm (upstream commit
; ecde3a2ae05061fe06469ed46df81a33b7de7d86).
;
; Syntax-only changes from upstream:
;   - Hex literals: $xx  -> &xx  (pyz80 uses & prefix, not $;
;     the location-counter uses of $ in "jr nc, $-4" are unchanged —
;     pyz80 supports those natively)
;   - No other changes; all mnemonics, labels, and logic are identical.
;
; The decoder is self-modifying (it patches PrevOffset+1) and uses IX,
; so it must run from RAM.
;
...
;   pyz80 --obj=dzx0_fast.bin dzx0_fast.asm
