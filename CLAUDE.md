...
**Tracking work (the registry).** The item/question registries are **generated**. The source of truth is `registry/items.yaml` and `registry/questions.yaml`; the `docs/notes/*-registry-*.md` files are output and must never be hand-edited (the `registry-sync` CI job fails on hand edits or stale generation). To track work:
- **New item:** `build/registry add --id $(build/registry next-id) --title "…" --desc "…" --status OPEN --owner agent [--parent iNN] [--ref …]`, then `make registry`. Commit the YAML and the regenerated `.md` together.
...
- Current state + milestone status: `docs/notes/m9-status.md` (active milestone) + `docs/notes/item-registry-open.md` / `question-registry-open.md` (open work; closed items in `item-registry-closed.md`; questions are transient — deleted once answered, no closed view).
