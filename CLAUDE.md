...
- **Inspect a record:** `build/registry view --id iNN|qNN` (add `--format json` for scripting) — prints the record plus computed dependents and priority rank. Use this instead of grepping the YAML.
- **New item (top-level):** `build/registry add --title "…" --desc "…" --status OPEN --owner agent [--dep iMM]… [--ref …]` — the id is **auto-allocated** (no `--id`; there is no `next-id`). For a **sub-item**, `build/registry split --parent iNN --title "…" [--desc …] [--status …] [--dep …]… [--ref …]` promotes the parent to an umbrella and adds a child whose id is **determined from the parent** (`iNN`→`iNNa`, `iNNx`→`iNNx-b1`). Then `make registry`; commit the YAML + regenerated `.md` together. **Create the item at request-time — before implementing — and mark it `IN_PROGRESS`** (ROADMAP handover-contract rule 1; many parallel asks ⇒ many `IN_PROGRESS` rows = the session completeness ledger, reconciled at wind-down via `build/registry in-progress`).
...
- **Edit a title / description:** `build/registry set-title --id iNN --title "…"` (items) / `build/registry set-desc --id iNN|qNN --desc "…"` (item description or question body), then `make registry`. Use these — **do not hand-edit `items.yaml`/`questions.yaml`** for title/description (the CLI validates the length limits, regenerates the views, and runs the dependency guard; a hand-edit bypasses all three).
